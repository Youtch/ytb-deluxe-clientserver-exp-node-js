...
const { GoogleCredentialsStore } = require('./google-credentials.service');

class GoogleService {
  constructor() {
    this.oauth2Client = null;
    this.userCredential = null;
    this.credentialsStore = new GoogleCredentialsStore();
  }
  ...
  restoreStoredCredentials() {
    try {
      const savedCredentials = this.credentialsStore.load();
      if (!savedCredentials) {
        return false;
      }
  ...
  saveCredentials(tokens) {
    try {
      const saved = this.credentialsStore.save(tokens);
      if (!saved) {
        return false;
      }
  ...
  clearStoredCredentials() {
    try {
      const cleared = this.credentialsStore.clear();
      if (!cleared) {
        return false;
      }
